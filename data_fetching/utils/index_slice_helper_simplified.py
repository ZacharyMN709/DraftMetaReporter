...
#  That would likely be None, slice, str, list[str], tuple[str, str], date, datetime

# region Name Slice
@functools.singledispatch
def get_name_slice(arg) -> Union[slice, list[str]]:
...
@get_name_slice.register(list[str])
def _get_name_slice_list(val: list[str]) -> list[str]:
    # Return a list as-is.
    return val


@get_name_slice.register(tuple[str, str])
def _get_name_slice_range(val: tuple[str, str]) -> slice:
...
# region Color Slice
@functools.singledispatch
def get_color_slice(arg) -> Union[slice, list[str]]:
    # By default, if we don't have a specific way of handling a parameter, raise an error.
...
@get_color_slice.register(list[str])
...
@get_color_slice.register(tuple[str, str])
def _get_color_slice_range(val: tuple[str, str]) -> slice:
    # If the parameter is a tuple, see if it has two elements.
    if len(val) == 2:
...
# region Date Slice
@functools.singledispatch
def _stringify_for_date_slice(arg) -> str:
    # By default, if we don't have a specific way of handling a parameter, raise an error.
...
@functools.singledispatch
def get_date_slice(arg) -> Union[slice, list[str]]:
    # By default, if we don't have a specific way of handling a parameter, raise an error.
...
@get_date_slice.register(list[Union[str, date, datetime]])
...
@get_date_slice.register(tuple[Union[str, date, datetime], Union[str, date, datetime]])
def _get_date_slice_range(val: tuple[Union[str, date, datetime], Union[str, date, datetime]]) -> slice:
